...
export const althea = defineChain({
  ...
  blockExplorers: {
    default: {
      name: "altheascan",
      url: "https://testnet.altheascan.net",
      apiUrl: "https://testnet.altheascan.net/api",
    },
  },
  contracts: {
    multicall3: {
...
const TRANSPORTS = {
  1: {
    http: http(
      "https://rpc.flashbots.net/?builder=flashbots&builder=f1b.io&builder=rsync&builder=beaverbuild.org&builder=builder0x69&builder=titan&builder=eigenphi&builder=boba-builder"
    ),
    tx: http(
      "https://rpc.flashbots.net/?builder=flashbots&builder=f1b.io&builder=rsync&builder=beaverbuild.org&builder=builder0x69&builder=titan&builder=eigenphi&builder=boba-builder"
    ),
  ...
  5: {
    http: http(`https://goerli.infura.io/v3/${process.env.INFURA_KEY}`),
    tx: http(`https://goerli.infura.io/v3/${process.env.INFURA_KEY}`),
    chain: goerli,
  },
  11155111: {
    http: http(`https://rpc.sepolia.org`),
    tx: http(`https://rpc.sepolia.org`),
    chain: sepolia,
  },
  7700: {
    http: http(`https://canto.gravitychain.io`),
    tx: http(`https://canto.gravitychain.io`),
    chain: canto,
  },
  81457: {
    http: http(`https://rpc.blast.io`),
    tx: http(`https://rpc.blast.io`),
    chain: blast,
  },
  168587773: {
    http: http(`https://sepolia.blast.io`),
    tx: http(`https://sepolia.blast.io`),
    chain: blastSepolia,
  },
  534351: {
    http: http(`https://sepolia-rpc.scroll.io`),
    tx: http(`https://sepolia-rpc.scroll.io`),
    chain: scrollSepolia,
  },
  534352: {
    http: http(`https://rpc.ankr.com/scroll/${process.env.ANKR_KEY}`),
...
const RELAY_SPEC = {
  ...
  tipRecv: [
    ...
    "0x09784d03b42581cfc4fc90a7ab11c3125dedeb86",
    "0xb4fdaf8e6636e7394f6ae768c5fa9d2e5bf6f0dc",
    "0x0412C7c846bb6b7DC462CF6B453f76D8440b2609",
    "0x30dA8589BFa1E509A319489E014d384b87815D89",
    "0x7580bFE88Dd3d07947908FAE12d95872a260F2D8",
    "0x9676519d99E390A180Ab1445d5d857E3f6869065",
...
if (process.env.DRY) console.log("Dry mode");
...
function parseTip(tip, chainId) {
  ...
  if (!RELAY_SPEC.tipRecv.includes(recv)) throw "Incorrect tip receiver";
...
async function getPrice(priceOf, relativeTo, chainId) {
  ...
  if (invert) [priceOf, relativeTo] = [relativeTo, priceOf];
  const pos = {
    base: priceOf,
    quote: relativeTo,
    poolIdx: CROC_CHAINS[chainId].poolIndex,
  };
...
async function estimateRelay(cmd) {
  ...
  let gas = await client.estimateContractGas({
    ...
    address: CROC_CHAINS[cmd.chainId].addrs.dex,
    abi: CROC_ABI,
...
async function sendRelayerTx(cmd, maxFeePerGas) {
  ...
  try {
    ...
    else return await wallet.writeContract(sim.request);
  } catch (e) {
