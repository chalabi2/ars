...
export function lpBaseTokens(pos, pool, percent = 100, human = false) {
  ...
  if (pos.positionType == "concentrated") {
    baseQty = baseTokenForConcLiq(
      pool.priceDecoded,
      floatToBigNum(liq),
      tickToPrice(pos.bidTick),
      tickToPrice(pos.askTick)
    ).toBigInt();
  } else {
  ...
  if (human)
    return getFormattedNumber(
      parseFloat(formatUnits(baseQty, pos._baseDecimals))
    );
  else return baseQty;
}
...
export function lpQuoteTokens(pos, pool, percent = 100, human = false) {
  ...
  if (human)
    return getFormattedNumber(
      parseFloat(formatUnits(quoteQty, pos._quoteDecimals))
    );
  else return quoteQty;
}
...
export function poolKey(position) {
  ...
  else return `${position.quote}_${position.base}_${position.poolIdx}`;
}
...
export function getSomeTokenForChain(chainId) {
  ...
  else if (chainId == 5) return "0xd87ba7a50b2e7e660f678a895e4b72e7cb4ccd9c";
  else if (chainId == 11155111)
    return "0x60bba138a74c5e7326885de5090700626950d509";
  else if (chainId == 7700) return "0xee602429ef7ece0a13e4ffe8dbc16e101049504c";
  ...
  else if (chainId == 12345)
    return "0x0412C7c846bb6b7DC462CF6B453f76D8440b2609";
...
export function ambientPosSlot(owner, base, quote, poolType) {
  ...
  const posKey = keccak256(
    encodePacked(["address", "bytes32"], [owner, poolHash])
  );
  return keccak256(
