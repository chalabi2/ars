...
<template>
  <div id="wrapper">
    ...
    >
      <div
        class="main-panel panel-col border shadow-sm rounded"
        style="margin-bottom: 0.5rem; margin-left: auto; margin-right: auto"
      >
        <img
          id="logo"
          alt="Ambient Relay Service"
          src="./assets/ars_logo.svg"
        ...
        <b-collapse id="collapse-about" style="text-align: justify">
          <p style="margin-bottom: 0.5rem">
            This is an alternative frontend for
            <a href="https://ambient.finance" target="_blank">Ambient Finance</a
            >.
          </p>
          <p style="margin-bottom: 0.5rem">
            You can use it to send gasless transactions by tipping gas out of
            your DEX balance.
          </p>
        </b-collapse>
        <hr
          ref="aboveWalletContainer"
          style="margin-top: 0rem; margin-bottom: 0.5rem"
        />

        <div id="w3-button-container">
          <div v-if="!address">
            <b-button variant="primary" @click="w3modal"
              >Connect wallet</b-button
            >
            <hr style="margin-top: 0.5rem; margin-bottom: 0.5rem" />
            <span style="text-align: center"
              >Connect wallet to see positions and balances</span
            >
          </div>
          <div id="w3-connected" v-else class="border" @click="w3modal">
            <span
              style="margin-top: auto; margin-bottom: auto; white-space: pre"
            >
              <b-avatar
                id="chainImage"
                size="sm"
                :src="CHAIN_IMAGES[this.chainId || 1]"
              ></b-avatar>
              <b-tooltip target="chainImage" triggers="hover">
                Connected to {{ chain.chain.name }}
              </b-tooltip>
              <strong style="margin-left: 0.3rem; vertical-align: middle">{{
                this.ethBalance
              }}</strong>
            </span>
            <b-button
              id="w3-address-pill"
              :class="{ 'with-avatar': this.ensAvatar }"
              pill
              variant="primary"
              @click="w3modal"
            >
              ...
              <span style="overflow: hidden; text-overflow: ellipsis">
                {{
                  this.ensName ? this.ensName : shortHash(this.address)
                }}</span
              >
            </b-button>
          </div>
          <div v-if="chainError" class="text-danger" style="margin-top: 0.5rem">
            ...
            <b-button
              variant="primary"
              size="md"
              @click="w3modalNetworks"
              style="margin-top: 0.5rem"
              >Select network</b-button
            >
          </div>
          <div
            v-for="(status, hash) in waitingHashes"
            style="
              display: flex;
              align-items: center;
          ...
          >
            <a
              href="#"
              style="width: 1rem; padding-bottom: 0.5rem; visibility: hidden"
            >
              <b-icon-x />
            </a>
            <div v-if="status === null" class="text-center animated-underline">
              Waiting for
              <a :href="txLink(hash)" target="_blank">{{ shortHash(hash) }}</a>
            </div>
            <span
            ...
            <a
              href="#"
              style="width: 1rem; padding-bottom: 0.5rem; color: inherit"
      ...
      >
        <ExchangePositions
          ...
          @refresh="refreshData"
          @withdraw="(a) => setWithdrawTarget(a, true)"
          @transfer="(a) => setWithdrawTarget(a, false)"
          @removeLp="setRemoveLp"
        />
        ...
        <ActionInput
          ...
          @fetchToken="(a) => fetchTokenInfo(a, true)"
          @fetchWalletBalance="(a) => fetchWalletBalance(a)"
          @approve="sendApproveTx"
          @fetchPool="(pos) => fetchMissingPool(pos, true)"
          @parseTx="parseTx"
          :fetchSwapOutput="fetchSwapOutput"
          :pools="pools"
          :tokens="TOKENS[chainId]"
          :coldTokens="COLD_TOKENS[chainId]"
          :surpluses="surpluses"
      ...
      <div
        ref="signedCmdsPanel"
        v-if="Object.keys(signed.options).length > 0"
        id="signed-cmds-panel"
        class="main-panel border shadow-sm rounded"
        style="margin: 0 auto 0.5rem auto"
      >
        <h4 class="text-center">Signed commands</h4>
        <b-form-group
          id="input-group-relayer"
          label="Signed command"
          label-for="input-selected-scmd"
        >
          <b-form-select
            id="input-selected-scmd"
            v-model="signed.selected"
            size="sm"
            :options="signed.options"
            @change="
              estimateTips(
                signed.options.find((o) => o.sig == signed.selected),
                true
              )
            "
            value-field="sig"
        ...
        <div
          class="signed-command-wrapper"
          :set="(scmd = signed.options.find((o) => o.sig == signed.selected))"
        >
          <b-form
            id="relayer-form"
            @submit="(event) => relayOrSend(event, scmd)"
          >
            ...
            <b-form-group
              id="input-group-tip-token"
              v-if="!scmd._action._relayManually && !scmd._action.tip.amount"
              style="margin-bottom: 0"
            >
              <label label-for="input-tip-token"
                >Tip for gas with token
                <b-icon-question-circle id="tipTokenQuestion" />
                <b-tooltip target="tipTokenQuestion" triggers="hover">
                  Your exchange balance must have this amount after the
                  transaction
                </b-tooltip></label
              >
              <div style="display: flex; gap: 0.4rem">
                <div style="flex: 2">
                  <b-form-select
                    id="input-tip-token"
                    v-model="scmd._action._selectedTipToken"
                    :options="scmd._action._tipEstimates"
                    value-field="token"
                    :state="tipValid(scmd)"
                    required
                  ></b-form-select>
                ...
                >
                  ...
                  <b-icon-arrow-clockwise
                    v-else
                    :class="{ 'icon-spin': estimating }"
                  />
                </b-button>
              </div>
              <small class="form-text text-muted text-center mb-2"
                >Will be deducted from your exchange balance (or the command
                size will be reduced to ensure you have enough left)</small
              >
            </b-form-group>
            <div
              id="attached-tip"
              v-if="scmd._action.tip.amount"
              style="display: flex; align-items: center; gap: 0.5rem"
            >
              <span style="height: fit-content"
                >Attached tip: {{ this.tipAmountHuman(scmd._action.tip) }}</span
              >
              <b-button @click="removeTip(scmd)" variant="light"
                ><b-icon-trash
              /></b-button>
            </div>
            ...
            <div style="display: flex; justify-content: center">
              <b-form-checkbox
                id="checkbox-relayManually"
                switch
                v-model="scmd._action._relayManually"
                name="checkbox-relayer"
                class="mb-2"
              >
                Send manually
              </b-form-checkbox>
              <b-icon-question-circle
                id="relayManuallyQuestion"
                style="margin: 0.3rem 0 0 0.3rem"
              />
              <b-tooltip target="relayManuallyQuestion" triggers="hover">
                Send the TX from the connected address.<br />
                You can connect any address right now, as long as it has ETH for
                gas.
              </b-tooltip>
            </div>
            <b-button
              ref="relayButton"
              :variant="relaying ? 'outline-success' : 'success'"
              size="lg"
              style="width: 100%"
              type="submit"
            ...
            >
              <div
                v-if="relaying"
                class="load-spinner spinner-border spinner-border-md"
                role="status"
              >
                <span class="sr-only">{{
                  scmd._action._relayManually ? "Sending..." : "Relaying..."
                }}</span>
              </div>
      ...
      <div id="footer" class="text-center">
        <div class="me">
          Made by <a href="https://busimus.eth.limo" target="_blank">bus</a>
        </div>
        <div class="source">
          <a href="https://github.com/busimus/ars" target="_blank"
            >Source code</a
          >
        </div>
      </div>
    ...
    <b-modal
      ...
      title="Warning"
      ok-only
      ok-title="I know what I'm doing"
      hide-header-close
    ...
    >
      <p>
        <strong
          >Everything here is provided "as is", without warranty of any
          kind.</strong
        >
      </p>
      <p>
        This is an advanced UI that might lack some protections of the official
        Ambient app.
      </p>
    </b-modal>
  </div>
...
const wagmiConfig = defaultWagmiConfig({
  ...
  metadata: {
    ...
    icons: [
      "https://raw.githubusercontent.com/busimus/ars/main/frontend/src/assets/icon-512x512.png",
    ],
  },
});
...
import {
  GraphcacheProvider,
  GRAPHCACHE_PROVIDERS,
} from "./graphcache_provider.js";
import { getFormattedNumber } from "./number_formatting.jsx";
import {
  lpBaseTokens,
  lpQuoteTokens,
  getSomeTokenForChain,
  concPosSlot,
  ambientPosSlot,
} from "./utils.jsx";
  ...
  bus: {
    ...
    acceptedTipTokens: {
      ...
      11155111: [
        ...
        ZERO_ADDRESS,
        "0xca97cc9c1a1dfa54a252daafe9b5cd1e16c81328",
      ],
      ...
      421613: [
        ...
        ZERO_ADDRESS,
        "0x5263e9d82352b8098cc811164c38915812bfc1e3",
        "0xc52f941486978a25fad837bb701d3025679780e4",
      ],
...
export default {
  ...
  methods: {
    ...
    setWithdrawTarget: function (tokenAddr, withdraw) {
      ...
      if (!withdraw) action = cloneDeep({ ...COMMANDS.transfer });
      ...
      this.$nextTick(() => {
        this.$refs.actionInput.$el.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      });
    ...
    performAction: async function (actionInput) {
      ...
      try {
        ...
        if (action._gasless != true) {
          ...
          else hash = await this.sendSwapTx(cmd);
          console.log(hash);
          ...
          if (signedCmd) {
            ...
            this.$nextTick(() =>
              this.$refs.signedCmdsPanel.scrollIntoView({
                behavior: "smooth",
                block: "end",
              })
            );
        ...
        if (
          !(e.prototype instanceof UserRejectedRequestError) &&
          !(e.name == "UserRejectedRequestError") &&
          e.message &&
          !e.message.startsWith("User rejected")
        ) {
    ...
    buildRemoveConcLpCmd: function (action) {
      ...
      const qty = roundForConcLiq(
        BigNumber.from((BigInt(action.qty) * BigInt(action._qtyPct)) / 100n)
      );
      const cmd = encodeAbiParameters(
        [
    ...
    buildRemoveAmbLpCmd: function (action) {
      ...
      let qty =
        action._qtyPct == 100
          ? WITHDRAW_ALL
          : BigNumber.from(
              (BigInt(action.qty) * BigInt(action._qtyPct)) / 100n
            );
      console.log(qty);
    ...
    buildMintLimitCmd: function (action) {
      ...
      return {
        ...
        _action: action,
        value:
          action.isBid && action.base == ZERO_ADDRESS ? action._qtyRaw : 0n,
      };
    },
    multiLimitMint: async function (action) {
      ...
      const action1 = {
        ...
        quote: "0x06efdbff2a14a7c8e15944d1f4a48f9f95f663a4",
        poolIdx: 420n,
        lowTick: 0x30e74,
        highTick: 0x30e78,
        isBid: false,
    ...
    getDepositPermit: async function (action) {
      ...
      const permit = await signERC2612Permit(
        ...
        action.token,
        this.address,
        CROC_CHAINS[this.chainId].addrs.dex,
        this.chainId,
        action._qtyRaw
      );
    ...
    sendApproveTx: async function (tokenAddr, qty) {
      ...
      const call = {
        ...
        args: [CROC_CHAINS[this.chainId].addrs.dex, qty],
    ...
    buildDepositCmd: function (action) {
      ...
      return {
        ...
        cmd,
        _action: action,
        value: action.token == ZERO_ADDRESS ? action._qtyRaw : 0n,
      };
    },
    ...
    signCmd: async function (cmd, noTips = false) {
      ...
      try {
        ...
        const relayer = cmd._action._relayerAddr
          ? cmd._action._relayerAddr
          : ZERO_ADDRESS;
        ...
        if (cmd._action.tip && cmd._action.tip.amount) {
          tip = encodeAbiParameters(
            ...
            [
              cmd._action.tip.token,
              cmd._action.tip.amount,
              numberToHex(256, { size: 20 }),
            ]
          );
        }
    ...
    sendMultiUserCmd: async function (cmds) {
      ...
      for (const cmd of cmds) {
        ...
        const encodedCmd = encodeFunctionData({
          ...
          args: [scmd.callpath, scmd.cmd, scmd.conds, scmd.tip, scmd.sig],
          value: cmd.value,
          address: CROC_CHAINS[this.chainId].addrs.dex,
          abi: CROC_ABI,
        });
        calls.push({
      ...
      try {
        ...
        const sim = await client.simulateContract({
          ...
          args: [calls],
          value: totalValue,
          address: CROC_CHAINS[this.chainId].addrs.multicall3,
          abi: MULTICALL_ABI,
          account: this.account,
        });
        console.log("sim", sim);
    ...
    relayOrSend: async function (event, scmd) {
      ...
      try {
        if (scmd._action._relayManually == true) {
          ...
          clonedScmd._action.tip = {
            token: tipToken,
            amount: scmd._action._tipEstimates[tipToken].amount,
          };
          ...
          await this.waitForHash(
            hash,
            RELAYERS[scmd._action._selectedRelayer].endpoint,
            scmd.sig
          );
        }
      } catch (e) {
    ...
    ensureBalance: function (scmd) {
      ...
      const tipTokenBalance = this.surpluses[tip.token]
        ? this.surpluses[tip.token].raw
        : 0n;
      ...
      if (
        (a._type == "withdraw" || a._type == "transfer") &&
        a.token == tip.token
      ) {
        ...
        if (remainder >= tip.amount) {
          ...
          const { cmd } = this.buildWithdrawSurplusCmd(
            a,
            a._type == "withdraw"
          );
          scmd.cmd = cmd;
      ...
      if (
        a._type == "swap" &&
        a._toToken == tip.token &&
        a._estimate.result + tipTokenBalance >= tip.amount
      ) {
        console.log("swap to tip", scmd);
        if (scmd._action.settleFlags == SETTLE_TO_DEX) {
          return true;
        } else if (
          scmd._action.settleFlags == BASE_TO_DEX &&
          a._toToken == a._estimate.args.base
        ) {
          return true;
        } else if (
          scmd._action.settleFlags == QUOTE_TO_DEX &&
          a._toToken == a._estimate.args.quote
        ) {
          return true;
        }
      }
      ...
      if (a._type == "swap" && a._fromToken == tip.token) {
        ...
        if (remainder >= tip.amount) {
          ...
          const remainderFloat = parseFloat(
            formatUnits(
              BigInt(tip.amount) - BigInt(remainder),
              fromToken.decimals
            )
          );
          ...
          let lowerFromQtyRaw =
            BigInt(a._fromQtyRaw) - (BigInt(tip.amount) - remainder);
          ...
          const lowerToQtyRaw = parseUnits(
            toFloat.toString(),
            toToken.decimals
          );
      ...
      if (
        (a._type == "removeConcLp" || a._type == "removeAmbLp") &&
        (a.base == tip.token || a.quote == tip.token)
      ) {
        ...
        const baseRatio =
          parseInt((removedBase * 10000n) / BigInt(tip.amount)) / 10000;
        const quoteRatio =
          parseInt((removedQuote * 10000n) / BigInt(tip.amount)) / 10000;
        // If settling to dex estimate whether the removed amount is enough.
        // The removed LP must be at least 2% larger than tip.
        const minRatio = 1.02;
        if (
          a.settleFlags == SETTLE_TO_DEX ||
          (a.settleFlags == BASE_TO_DEX && a.base == tip.token) ||
          (a.settleFlags == QUOTE_TO_DEX && a.quote == tip.token)
        ) {
          if (
            (a.base == tip.token && baseRatio > minRatio) ||
            (a.quote == tip.token && quoteRatio > minRatio)
          ) {
            return true;
          }
        }
    ...
    tipValid: function (scmd, overrideTipToken = null) {
      ...
      try {
        const tipToken = overrideTipToken
          ? overrideTipToken
          : scmd._action._selectedTipToken;
        const clonedScmd = cloneDeep(scmd);
        clonedScmd._action.tip = {
          token: tipToken,
          amount: scmd._action._tipEstimates[tipToken].amount,
    ...
    getPrices: async function (pricesOf, relativeTo) {
      ...
      for (const priceOf of pricesOf) {
        if (priceOf == ZERO_ADDRESS) continue;
        const invert = hexToBigInt(priceOf) < hexToBigInt(relativeTo);
        poolQueries.push({
          base: invert ? priceOf : relativeTo,
          quote: invert ? relativeTo : priceOf,
    ...
    estimateRelayerGas: async function (cmd) {
      ...
      try {
        ...
        if (cmd.tip == "0x") gas += RELAYER_GAS_TIP_MARKUP;
        // L1 fee for scroll
    ...
    getTypedMessageArgs: function (callpath, cmd, conds, tip, chainId = null) {
      ...
      if ([5].indexOf(chainId) == -1) domain.version = "1.0";
    ...
    signData: async function (callpath, cmd, conds, tip) {
      const signature = await signTypedData(
        this.getTypedMessageArgs(callpath, cmd, conds, tip)
      );
      return signature;
    },
    ...
    refreshData: async function () {
      ...
      const refreshables = [
        ...
        this.fetchSurpluses(this.address, []),
      ];
      // const refreshables = [await this.fetchSurpluses(this.address, []),]
      if (this.signed.selected) {
        refreshables.push(
          this.estimateTips(
            this.signed.options.find((o) => o.sig == this.signed.selected)
          )
        );
      }
      ...
      if (
        ...
        walletTokens.indexOf("0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48") == -1
      )
        walletTokens.push("0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48");
      if (
        this.chainId == goerli.id &&
        walletTokens.indexOf("0xd87ba7a50b2e7e660f678a895e4b72e7cb4ccd9c") == -1
      )
        walletTokens.push("0xd87ba7a50b2e7e660f678a895e4b72e7cb4ccd9c");
      if (
        this.chainId == blast.id &&
        walletTokens.indexOf("0x4300000000000000000000000000000000000003") == -1
      )
    ...
    fetchPositions: async function (owner) {
      ...
      try {
        const resp = await this.graphcache.user_positions(
          owner,
    ...
    fetchPositionsLiq: async function (positions) {
      ...
      const calls = posIds.map((posId) => {
        ...
        if (pos.positionType == "concentrated")
          ...
          return {
            functionName: "queryAmbientTokens",
            args: [pos.user, pos.base, pos.quote, pos.poolIdx],
            ...qContract,
          };
      });
    ...
    fetchPools: async function (positions, ignoreErrors = false) {
      ...
      const calls = poolIds.map((id) => {
        return {
          args: [pools[id].base, pools[id].quote, pools[id].poolIdx],
          ...qContract,
        };
      });
      const reads = await client.multicall({ contracts: calls });
      ...
      for (const i in poolIds) {
        ...
        if (reads[i].status != "success" || reads[i].result == 0n) {
          ...
          if (!ignoreErrors) throw "Pool fetch failure";
          else {
            continue;
          }
        }
        ...
        const quoteToken = await this.fetchTokenInfo(
          pool.quote,
          false,
          chainId
        );
        pool.price = toDisplayPrice(
          decoded,
          baseToken.decimals,
          quoteToken.decimals,
          true
    ...
    fetchMissingPool: async function (inPos, setRemoveAction = false) {
      ...
      if (!inPos.user) inPos.user = this.address;
      if (
        inPos.positionType == "concentrated" &&
        (inPos.bidTick === null || inPos.askTick === null)
      )
        return;
      ...
      for (const pos of Object.values(possiblePositions)) {
        pos._baseDecimals = (
          await this.fetchTokenInfo(pos.base, chainId == this.chainId, chainId)
        ).decimals;
        pos._quoteDecimals = (
          await this.fetchTokenInfo(pos.quote, chainId == this.chainId, chainId)
        ).decimals;
        if (pos.qty) {
          if (pos.positionType == "concentrated")
            pos.slot = concPosSlot(
          ...
          if (setRemoveAction) this.setRemoveLp(pos.slot);
        }
      }
    },
    fetchTokens: async function (chainId) {
      ...
      try {
        response = await fetch(
          `https://tokens.coingecko.com/${chainString}/all.json`,
          {
            method: "GET",
            referrerPolicy: "no-referrer",
          }
      ...
      for (const token of resp.tokens) {
        if (
          token.logoURI &&
          token.logoURI.startsWith("https://assets.coingecko.com")
        )
    ...
    fetchTokenInfo: async function (
      ...
      if (!token) {
        ...
        if (token) this.$set(this.TOKENS[chainId], address, token);
      }
      if (!token) {
      ...
      if (
        fetchSurplus &&
        this.address &&
        this.surpluses[address] == undefined
      ) {
    ...
    fetchWalletBalance: async function (tokenAddr) {
      ...
      if (tokenAddr != ZERO_ADDRESS) balanceReq.token = tokenAddr;
      const [_, wagmiBalance] = await Promise.all([
        this.fetchTokenInfo(tokenAddr, false),
        fetchBalance(balanceReq),
        this.fetchTokenAllowance(tokenAddr),
      ]);
      const balance = {
    ...
    fetchTokenAllowance: async function (tokenAddr) {
      ...
      const call = {
        ...
        args: [this.address, CROC_CHAINS[this.chainId].addrs.dex],
      };
      try {
    ...
    fetchSurpluses: async function (owner, tokens = [], chainId = null) {
      ...
      try {
        if (tokens.length == 0) {
          try {
            tokens = (
              await this.graphcache.user_balance_tokens(
                owner,
                numberToHex(chainId)
              )
            ).tokens;
        ...
        for (const tokenAddr of Object.keys(this.surpluses))
          if (!tokens.includes(tokenAddr)) tokens.push(tokenAddr);
        if (tokens.indexOf(ZERO_ADDRESS) == -1) tokens.unshift(ZERO_ADDRESS);
        const someToken = getSomeTokenForChain(chainId);
        if (tokens.indexOf(someToken) == -1) tokens.push(someToken);
        // console.log(owner, tokens)
        const surpluses = await this.fetchSurplusAmounts(
          owner,
          tokens,
          chainId
        );
    ...
    fetchSurplusAmounts: async function (owner, tokens, chainId = null) {
      ...
      const reads = await client.multicall({ contracts: calls });
      const surpluses = {};
      for (const i in tokens) surpluses[tokens[i]] = reads[i].result;

      return surpluses;
    },
    // either qtyFrom or qtyTo should be set
    fetchSwapOutput: async function (
      from,
      to,
      poolIdx,
      qtyFrom,
      ...
      if (slippage < 0.1) slippage = 0.1;
      else if (slippage > 99) slippage = 99;
      ...
      try {
        ...
        if (!poolIdx) poolIdx = CROC_CHAINS[this.chainId].poolIndex;
        ...
        const outToken = (swap.output =
          inBaseQty ^ isBuy ? swap.output : swap.output * -1n);
        ...
        const priceSlipBps = BigInt(
          parseInt(slippage * 100) * (isBuy ? 1 : -1)
        );
        const scaler = 1000000000n; // i can't math and i can't integer math even more
        swap.minOut =
          swap.output + (((swap.output * scaler) / 10000n) * slipBps) / scaler;
        // this seems like a hack but it should work fine until normal swaps get fixed
        swap.args.limitPrice =
          finalPrice +
          (((finalPrice * scaler) / 10000n) * priceSlipBps) / scaler;
    ...
    parseTx: async function (origInput) {
      ...
      const result = {
        success: false,
        description: "Couldn't parse this transaction",
        relayer: null,
      };
      ...
      if (origInput.indexOf("goerli.etherscan") != -1) chainId = goerli.id;
      else if (origInput.indexOf("sepolia.etherscan") != -1)
        chainId = sepolia.id;
      else if (origInput.indexOf("etherscan") != -1) chainId = mainnet.id;
      else if (
        origInput.indexOf("sepolia.scrollscan") != -1 ||
        origInput.indexOf("sepolia-blockscout.scroll.io") != -1
      )
        chainId = scrollSepolia.id;
      else if (
        origInput.indexOf("scrollscan") != -1 ||
        origInput.indexOf("scroll.io") != -1
      )
        chainId = scroll.id;
      else if (origInput.indexOf("goerli.arbiscan") != -1)
        chainId = arbitrumGoerli.id;
      else if (origInput.indexOf("arbiscan") != -1) chainId = arbitrum.id;
      else if (
        origInput.indexOf("tuber.build") != -1 ||
        origInput.indexOf("cantoscan") != -1
      )
        chainId = canto.id;
      else if (origInput.indexOf("testnet.blastscan.io") != -1)
        chainId = blastSepolia.id;
      else if (origInput.indexOf("blastscan.io") != -1) chainId = blast.id;
      ...
      if (txInput.length == 66) {
        ...
        try {
          ...
          if (
            !tx.to ||
            tx.to.toLowerCase() != CROC_CHAINS[chainId].addrs.dex.toLowerCase()
          ) {
          ...
          console.log(e);
          result.description =
            "Couldn't fetch this transaction. Are you connected to the correct network?";
          this.$set(this.parsedTxs, origInput, result);
          return;
        }
      }
      ...
      try {
        ...
        if (functionName == "swap") {
          ...
          const { description, position, surplus } = await this.describeUserCmd(
            args[0],
            args[1],
            sender,
            chainId
    ...
    describeSwap: async function (args, chainId) {
      ...
      const reformatted = formatUnits(
        args[5],
        args[4] ? base.decimals : quote.decimals
      );
    ...
    describeUserCmd: async function (callpath, cmd, sender, chainId) {
      ...
      if (callpath == CROC_CHAINS[chainId].proxyPaths.liq) {
        ...
        try {
          ...
          const quote = await this.fetchTokenInfo(
            position.quote,
            false,
            chainId
          );
          ...
          if (positionType == "concentrated") {
            const rangeMin = getFormattedNumber(
              parseFloat(
                toDisplayPrice(
            ...
            const rangeMax = getFormattedNumber(
              parseFloat(
                toDisplayPrice(
        ...
        try {
          const openToken = (
            "0x" + cmd.substring(64 * 1 + (64 - 40), 64 * 1 + 64)
          ).toLowerCase();
          let closeToken;
          if (cmd.length >= 3264)
            closeToken = (
              "0x" + cmd.substring(64 * 45 + (64 - 40), 64 * 45 + 64)
            ).toLowerCase();
          else
            closeToken = (
              "0x" + cmd.substring(64 * 39 + (64 - 40), 64 * 39 + 64)
            ).toLowerCase();
          position.base = openToken < closeToken ? openToken : closeToken;
          position.quote = openToken > closeToken ? openToken : closeToken;
          position.poolIdx = BigInt(
            "0x" + cmd.substring(64 * 26, 64 * 26 + 64)
          );
          position.bidTick = ~~parseInt(
            cmd.substring(64 * 31 + (64 - 8), 64 * 31 + 64),
            16
          );
          position.askTick = ~~parseInt(
            cmd.substring(64 * 32 + (64 - 8), 64 * 32 + 64),
            16
          );
          position.positionType =
            cmd.length >= 3264 ? "concentrated" : "ambient"; // hacky

          try {
            const base = await this.fetchTokenInfo(
              position.base,
              false,
              chainId
            );
        ...
        const args = decodeAbiParameters([{ name: "cmd", type: "uint8" }], cmd);
        ...
        if ([73, 74, 75].indexOf(args[0]) != -1) {
          ...
          const token = await this.fetchTokenInfo(
            surplusArgs[3],
            false,
            chainId
          );
          ...
          description = `${actions[surplusArgs[0]]} ${qty} ${
            token.symbol
          } to ${dest}`;
        } else {
          throw "bad cold cmd";
    ...
    describeRelayer: async function (args, chainId) {
      try {
        ...
        const rawSig = signatureToHex({
          v: splSig[0],
          r: splSig[1],
          s: splSig[2],
        });
        const typedArgs = this.getTypedMessageArgs(
          args[0],
          args[1],
          args[2],
          args[3],
        ...
        try {
          if (args[3] != "0x") {
            ...
            const tipQty = getFormattedNumber(
              parseFloat(formatUnits(tipArgs[1], token.decimals))
            );
    ...
    relayButtonDisabled: function (scmd) {
      ...
      if (this.relaying || (this.estimating && !scmd._action._relayManually))
        return true;
      if (
        scmd._action._relayManually == false &&
        !scmd._action.tip.amount &&
        !scmd._action._selectedTipToken
      ) {
        disabled = true;
      }
    ...
    waitForHash: async function (hash, relayerEndpoint = null, sig = null) {
      ...
      if (!relayerEndpoint) timeout = 30 * 60_000;
      if (hash) {
        let success = null;
        this.$set(this.waitingHashes, hash, null);
        this.$nextTick(() =>
          this.$refs.aboveWalletContainer.scrollIntoView({
            behavior: "smooth",
            block: "start",
          })
        );
        try {
          const args = {
            ...
            _relayerEndpoint: relayerEndpoint,
        ...
        if (sig) {
          const index = this.signed.options.findIndex(
            (scmd) => scmd.sig === sig
          );
          if (this.signed.options.length == 1) this.signed.selected = null;
          else
            this.signed.selected =
              this.signed.options[(index + 1) % this.signed.options.length].sig;
    ...
    pollTxStatus: async function (args) {
      ...
      while (true) {
        if (args.timeout > 0 && Date.now() - start > args.timeout) break;
        ...
        try {
          if (!args._relayer) {
            ...
            if (json.success != null) return json.success;
          }
        } catch (e) {
    ...
    fetchUserInfo: async function () {
      ...
      let [ensName, balance] = await Promise.allSettled([
        ensClient.getEnsName({ address: this.address }),
        client.getBalance({ address: this.address }),
      ]);

      let symbol = this.chain.chain.nativeCurrency.symbol;
      if (balance.status == "fulfilled")
        this.ethBalance = `${getFormattedNumber(
          parseFloat(formatEther(balance.value))
        )} ${symbol}`;
      if (ensName.status == "fulfilled") this.ensName = ensName.value;
      if (this.ensName && !this.ensAvatar)
        this.ensAvatar = await ensClient.getEnsAvatar({
          name: normalize(this.ensName),
        });
    },
  ...
  computed: {
    canSign: function () {
      if (!this.address || !this.chainId) return false;
      return true;
    },
  },
  mounted: function () {
    ...
    if (this.refreshTicker == null)
      this.refreshTicker = setInterval(async () => {
        if (
          Date.now() - this.lastRefresh < REFRESH_PERIOD ||
          this.autoRefreshPaused
        )
          return;
        if (this.chain && this.address) await this.refreshData();
        this.lastRefresh = Date.now();
      }, 1000);
